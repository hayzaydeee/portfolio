...
function HtmlBlock({ html }: { html: string }) {
  ...
  return (
    <div
      className="prose prose-sm max-w-none text-(--workshop-text)
...
export function ProjectFile({ project }: ProjectFileProps) {
  return (
    <div
      className="p-8 max-w-3xl mx-auto overflow-y-auto h-full text-(--workshop-text)"
    >
      {/* Header */}
      <div className="mb-8">
        ...
        {/* External links */}
        <div className="flex items-center gap-4 text-xs text-(--workshop-syntax)">
          {project.live_url && (
            <Link
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {project.stack.map((tech) => (
              <code
                key={tech}
                className="px-2 py-0.5 rounded text-xs font-mono text-(--workshop-syntax)"
                style={{
